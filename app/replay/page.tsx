...
export default function ReplayPage() {
  ...
  // Build SVG path with adaptive gap threshold — M-break when a step is
  // > gapMult × median step size (handles any circuit, any GPS resolution)
  const buildPath = useCallback((pts: [number, number][], gapMult = 6): string => {
    if (pts.length < 2) return "";
    const sv = pts.map(([rx, ry]) => toSvg(rx, ry));
    // Compute median consecutive gap in SVG space
    const gaps: number[] = [];
    ...
    const maxGap2 = (median * gapMult) ** 2;   // squared for cheap comparison
  ...
  return (
    <main className="min-h-screen pt-20 pb-8 px-4" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="max-w-[1440px] mx-auto flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
          ...
          <div
            ...
            style={{ background: "rgba(4,4,4,0.97)", borderColor: "var(--border)", minHeight: 480 }}
          >
            ...
            >
              {/* Track — single clean circuit outline */}
              {trackPath && (
              ...
              {/* Cars — pre-rendered for all driver codes, positions updated via DOM refs at 60fps */}
              {ALL_DRIVER_CODES.map(code => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Pit indicator — hidden by default, toggled via pitRingRefs */}
                    <circle
                      ...
                      style={{ display: "none" }}
                    />
                    {/* Driver code — two-pass for outline effect */}
